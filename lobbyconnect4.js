...
function setupBetButtons() {
    ...
    createGameBtn?.addEventListener('click', async () => {
        ...
        await createConnectFourGame(selectedBet);
        createGameBtn.disabled = false;
    });
}

// --- Connect Four Game Creation ---
async function createConnectFourGame(bet) {
    if (!validateGameCreation(bet)) return;

    displayMessage(createGameStatusEl, 'Creating Connect Four game...', 'info');

    try {
        const gameCode = generateGameCode();
        const users = JSON.parse(localStorage.getItem('user'));
        
        // Initialize empty Connect Four board
        ...
        const { data: createdGameData, error } = await supabase
            ...
            .insert([{
                ...
                board: board,
                current_turn: users.phone  // Creator goes first
        ...
        window.location.href = `${BASE_URL}/connect4.html?code=${createdGameData.code}`;
    } catch (error) {
...
// --- Game Joining ---
async function joinConnectFourGame(gameCode, gameBet) {
    if (!validateJoinGame(gameBet)) return;

    displayMessage(createGameStatusEl, 'Joining Connect Four game...', 'info');
    ...
    try {
        ...
        const { data: gameData, error: fetchError } = await supabase
            .from('connect_four_games')
            .select('creator_phone, opponent_phone, bet, is_private, status, board')
            .eq('code', gameCode)
            .single();
        ...
        const { error: joinError } = await supabase
            ...
            .update({
                ...
                status: 'ongoing',
                current_turn: gameData.creator_phone  // Creator goes first
            })
            .eq('code', gameCode);

        if (joinError) throw joinError;

        window.location.href = `${BASE_URL}/connect4.html?code=${gameCode}`;
    } catch (error) {
...
function displayAvailableGames(games) {
    ...
    document.querySelectorAll('.join-btn').forEach(button => {
        button.addEventListener('click', async () => {
            ...
            await joinConnectFourGame(gameCode, gameBet);
        });
    });
}
...
async function handleJoinPrivateGame() {
    ...
    try {
        ...
        await joinConnectFourGame(gameCode, gameData.bet);
    } catch (error) {
...
function setupRealtimeUpdates() {
    ...
    supabaseChannel = supabase
        .channel('connect_four_games_changes')
        .on(
            'postgres_changes',
            {
                event: '*',
