...
async function createGame(bet) {
    ...
    try {
        ...
        window.location.href = `${BASE_URL}/chess?code=${createdGameData.code}&color=white`;
    } catch (error) {
...
async function joinGame(gameCode, gameBet) {
    ...
    try {
      ...
      const { data: gameData, error: fetchError } = await supabase
        ...
        window.location.href = `${BASE_URL}/chess?code=${gameCode}&color=black`;
    } catch (error) {
...
async function handleJoinPrivateGame() {
    ...
    try {
        ...
        // First check if the game exists and is private
        const { data: gameData, error: fetchError } = await supabase
            .from('chess_games')
            .select('white_phone, black_phone, bet, is_private, status')
            .eq('code', gameCode)
            .single();
